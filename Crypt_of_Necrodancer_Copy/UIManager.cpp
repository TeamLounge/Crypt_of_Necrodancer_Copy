...
void UIManager::setHeartBeat(int heartBeatNum)
{
	...
	for (int i = 0; i < heartBeatNum; ++i)
	{
		...
		_heartBeatRight->init("heartBeat", PointMake(CAMERAMANAGER->getCameraRIGHT() + 400 - i * _heartBeatInterval,
	...
	for (int i = 0; i < heartBeatNum; ++i)
	{
		...
		_heartBeatLeft->init("heartBeat", PointMake(CAMERAMANAGER->getCameraLEFT() - 400 + i * _heartBeatInterval,
			CAMERAMANAGER->getCameraBOTTOM() - 100));
